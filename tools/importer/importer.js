...
export default {
    transform: ({
        ...
        if (title && description) {
            const cells = [['Leadspace (document)'], [title.outerHTML + description.innerHTML],];
        ...
        if (pdfUrlEl) {
            ...
            const pdfCells = [['PDF Viewer'], ['Document Link', pdfUrl],];
        ...
        if (rrText) {
            ...
            const relatedResourcesCells = [['Related Resources'], [rrLinks],];
        ...
        if (ctaText && ctaHeading && ctaLinkText) {
            ...
            const ctaCells = [['CTA'], [ctaHeading.outerHTML + ctaTextInner + ctaLink],];
        ...
        results.push({
            element: main, path: new URL(url).pathname,
        });
